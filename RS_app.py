...
st.set_page_config(
    layout="wide", 
    page_title="ابزار تحلیل تصاویر ماهواره‌ای - تشخیص تغییرات ساختمانی"
)

# اضافه کردن CSS فارسی
st.markdown("""
<style>
...
with tab1:
    ...
    st.markdown("<h1 style='color: #667eea;'>🗺️ انتخاب منطقه مورد نظر و دوره‌های زمانی</h1>", unsafe_allow_html=True)
    
    # کارت‌های راهنما
    st.markdown("""
    <div class='guide-box'>
        <h3>📋 راهنمای استفاده:</h3>
        <ul style='text-align: right; padding-right: 25px;'>
            <li>برای نتایج بهینه، مناطقی کوچکتر از 40 کیلومتر مربع انتخاب کنید</li>
            <li>از ابزار ترسیم روی نقشه برای انتخاب منطقه استفاده کنید</li>
            <li>مناطق بزرگتر با استفاده از تکنیک کاشی‌کاری پردازش می‌شوند</li>
        </ul>
    </div>
    """, unsafe_allow_html=True)
    
    # کارت‌های آماری
    if 'drawn_polygons' in st.session_state:
        col1, col2, col3 = st.columns(3)
        
        with col1:
            st.markdown(f"""
            <div class='stat-card success'>
                <h3 style='color: #27ae60; margin: 0;'>مناطق ذخیره شده</h3>
                <p style='font-size: 36px; font-weight: 700; margin: 10px 0;'>
                    {len(st.session_state.drawn_polygons)}
                </p>
            </div>
            """, unsafe_allow_html=True)
    ...
    with time_selection_container:
        ...
        st.markdown("""
        <div class='guide-box'>
            <h3 style='color: #667eea;'>📅 انتخاب دوره‌های زمانی برای تحلیل</h3>
        </div>
        """, unsafe_allow_html=True)
...
if 'reconstructed_before_image' in st.session_state:
        ...
        <div class='stat-card success'>
            <h3>✅ وضعیت پردازش</h3>
            <p>تصویر قبل با موفقیت پردازش شد</p>
        </div>
        """, unsafe_allow_html=True)
    ...
    st.markdown("""
    <div class='guide-box'>
        <h3>📋 مراحل پردازش خودکار:</h3>
        <ol style='text-align: right; padding-right: 25px;'>
            <li>دانلود تصویر Sentinel-2 از Google Earth Engine</li>
            <li>برش تصویر بر اساس منطقه انتخابی</li>
            <li>تقسیم به بخش‌های کوچکتر (Patches)</li>
            <li>طبقه‌بندی با مدل هوش مصنوعی</li>
            <li>بازسازی تصویر نهایی</li>
        </ol>
    </div>
    """, unsafe_allow_html=True)
    
    if st.button("⬇️ دانلود و پردازش تصویر", key="process_before"):
        st.info("🔄 در حال شروع دانلود...")
